...
PHPIZE=${1:?usage: build_yac.sh /path/to/phpize /path/to/php-config /path/to/output-dir optional-work-dir}
PHP_CONFIG=${2:?usage: build_yac.sh /path/to/phpize /path/to/php-config /path/to/output-dir optional-work-dir}
OUTPUT_DIR=${3:?usage: build_yac.sh /path/to/phpize /path/to/php-config /path/to/output-dir optional-work-dir}
WORK_DIR=${4:-${OUTPUT_DIR}/build-src}
YAC_VERSION=${YAC_VERSION:-master}
YAC_REPO=${YAC_REPO:-https://github.com/laruence/yac.git}
...
if test ! -d "${WORK_DIR}/.git"; then
  rm -rf "${WORK_DIR}"
  git clone --depth 1 --branch "${YAC_VERSION}" "${YAC_REPO}" "${WORK_DIR}"
else
  git -C "${WORK_DIR}" fetch --depth 1 origin "${YAC_VERSION}"
...
make -j"${MAKE_JOBS}"
cp modules/yac.so "${OUTPUT_DIR}/yac.so"
